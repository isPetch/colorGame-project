...
const colorss = [
  ...
  { yes: "bg-yellow-400", no: "bg-yellow-300"},
  { yes: "bg-orange-800", no: "bg-orange-700"},
  { yes: "bg-pink-800", no: "bg-pink-700" },
  { yes: "bg-red-300", no: "bg-red-200" },
  { yes: "bg-green-800", no: "bg-green-700" },
  { yes: "bg-pink-700", no: "bg-pink-600" }
]

let lucky1 = colorss[Math.floor(Math.random() * colorss.length)];
let lucky2 = colorss[Math.floor(Math.random() * colorss.length)];
let lucky3 = colorss[Math.floor(Math.random() * colorss.length)];
let lucky4 = colorss[Math.floor(Math.random() * colorss.length)];
...
const functionRandom1 = () => {
  for (let i = 0; i < 4; i++) {
    if (randomColor1.length < 3) {
      randomColor1.push(lucky1.yes)
    } else {
      randomColor1.push(lucky1.no)
    }
  }
}
functionRandom1()
...
const functionRandom2 = () => {
  for (let i = 0; i < 9; i++) {
    if (randomColor2.length < 8) {
      randomColor2.push(lucky2.yes)
    } else {
      randomColor2.push(lucky2.no)
    }
  }
}
functionRandom2()
...
const functionRandom3 = () => {
  for (let i = 0; i < 16; i++) {
    if (randomColor3.length < 15) {
      randomColor3.push(lucky3.yes)
    } else {
      randomColor3.push(lucky3.no)
    }
  }
}
functionRandom3()
...
const functionRandom4 = () => {
  for (let i = 0; i < 25; i++) {
    if (randomColor4.length < 24) {
      randomColor4.push(lucky4.yes)
    } else {
      randomColor4.push(lucky4.no)
    }
  }
}
functionRandom4()

//Swap the position of the color.
...
const changeColor1 = () => {
  score.value++
  lucky1 = colorss[Math.floor(Math.random() * colorss.length)]
...
const changeColor2 = () => {
  score.value++
  lucky2 = colorss[Math.floor(Math.random() * colorss.length)]
...
const changeColor3 = () => {
  score.value++
  lucky3 = colorss[Math.floor(Math.random() * colorss.length)]
...
const changeColor4 = () => {
  score.value++
  lucky4 = colorss[Math.floor(Math.random() * colorss.length)]
              ...
              <div class="flex w-full h-2/3  border-b-2 border-gray-500">
                <div class="flex flex-col w-1/2 h-2/3 m-auto pt-6 space-y-4">
                  <input type="text" placeholder="Type your name..." class="input input-bordered w-full max-w-xs "
                    v-model="player" />
                  <button class=" btn w-full normal-case" style="background-color: #111B2E"
                    @click="dsinone = !dsinone, time = 60 ">Let's start!</button>
                </div>
              </div>
              <div class="w-full h-1/3 ">
                <div class="flex flex-col w-1/2 h-2/3 m-auto pt-8">
                  <label for="my-modal" class=" btn w-full normal-case" style="background-color: #111B2E">How to
                    play</label>
                  ...
                  <div class="modal">
                    <div class="modal-box text-white" style="background-color: #304477">
                      ...
                      <p class="py-4">Click on the different shade of color in a short period of time. Once you find the
                        different shade, you click it and score one point if it is the different one.</p>
                ...
                <div class="flex w-full h-5/6  m-auto ">
                  <div class="flex w-56 h-56 space-x-1  m-auto " v-if="score >= 0 && score <= 4">
                    <div class="grid grid-cols-2 gap-1 m-auto">
                      <div v-for="random in randomPosition1" class="flex w-full h-full "> 
                        <div class="rounded-full w-28 h-28" :class="random" v-if="random === lucky1.no" @click="changeColor1()"> </div>
                        <div class="rounded-full w-28 h-28" :class="random" v-else=""> </div>
                      </div>
                    </div>
                  </div>
                  <div class="flex w-72 h-72 space-x-1  m-auto " v-if="score >= 5 && score <= 14">
                    <div class="grid grid-cols-3 gap-1 m-auto">
                      <div v-for="random2 in randomPosition2" class="flex w-full h-full ">
                        <div class="rounded-full w-24 h-24" :class="random2" v-if="random2 === lucky2.no " @click="changeColor2()"> </div>
                        <div class="rounded-full w-24 h-24" :class="random2" v-else=""> </div>
                      </div>
                    </div>
                  </div>
                  <div class="flex w-72 h-72 space-x-1  m-auto " v-if="score >= 15 && score <= 24">
                    <div class="grid grid-cols-4 gap-1 m-auto">
                      <div v-for="random in randomPosition3" class="flex w-full h-full "> 
                        <div class="rounded-full w-16 h-16" :class="random" v-if="random === lucky3.no" @click="changeColor3()"> </div>
                        <div class="rounded-full w-16 h-16" :class="random" v-else=""> </div>
                      </div>
                    </div>
                  </div>
                  <div class="flex w-80 h-80 space-x-1  m-auto " v-if="score >= 25">
                    <div class="grid grid-cols-5 gap-1 m-auto">
                      <div v-for="random in randomPosition4" class="flex w-full h-full "> 
                        <div class="rounded-full w-14 h-14" :class="random" v-if="random === lucky4.no" @click="changeColor4()"> </div>
                        <div class="rounded-full w-14 h-14" :class="random" v-else=""> </div>
                      </div>
                    </div>
                  </div>
                </div>
